...
public class LessonTimeServiceImpl extends ServiceImpl<LessonTimeMapper, LessonTime> implements ILessonTimeService {
    ...
    @Override
    public ResponseResult calAvailableTime(String uid, Integer status,LocalDate term) {
        ...
        for (MajorCourse majorCours : majorCourses) {
            selectLessonTimePart(majorCours, ban, notRequired, status,term);
        }
...
//                lessonTime.setLessonName()
    ...
    private void selectLessonTimePart(MajorCourse majorCourse, Set<LessonTimePart> ban
            , Set<LessonTimePart> notRequired, Integer status,LocalDate term) {
        //找出这个专业其他的课程
        ...
        if (majorCourse.getRequired() == 1) {
            ...
            addQueue(ban, courseIds, status,term);
        } else {
            List<Long> courseIds =
            ...
            addQueue(notRequired, courseIds, status,term);
        }

    }

    private void addQueue(Set<LessonTimePart> ban, List<Long> courseIds, Integer status,LocalDate term) {
        if (courseIds.isEmpty()) return;

        List<Long> lessonIds = Db.lambdaQuery(Lesson.class)
                .in(Lesson::getCourseId, courseIds).eq(Lesson::getTerm,term
                ).list().stream().map(Lesson::getId).collect(Collectors.toList());
